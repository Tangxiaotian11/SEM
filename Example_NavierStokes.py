...
∂ₜ[u, v] + Re([u, v]∘∇)[u, v]  = -∇p + ∇²[u, v] ∀t>0, x∈[0,L_x]×[0,L_y]
∇∘[u, v] = 0 ∀t>0, x∈[0,L_x]×[0,L_y]
...
while res > tol:
    n += 1
    # KIM-MOIN; Convection term OSEEN
    Conv = Re * (u @ C_x + v @ C_y)
...
# plot
x_plot, y_plot = np.meshgrid(np.linspace(0, L_x, 50), np.linspace(0, L_y, 50), indexing='ij')
...
# print y, u(x=0.5,y)
np.savetxt('Cavity.out', np.transpose([points[1, np.isclose(points[0], 0.5)], u[np.isclose(points[0], 0.5)]]),
           delimiter=',')
